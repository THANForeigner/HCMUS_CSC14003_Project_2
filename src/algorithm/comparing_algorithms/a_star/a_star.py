...
class PureAStarSolver(FutoshikiSolver):
    ...
    def has_conflict(self, state: FutoshikiState) -> bool:
        # Tối ưu: Dùng numpy indexing trực tiếp để tránh tạo list comprehension thừa
        for i in range(self.size):
    ...
    def h2_forward_checking(self, state: FutoshikiState) -> int:
        # H2 MỚI: Đếm tổng số giá trị đã bị loại trừ khỏi các ô trống.
        # Giá trị càng lớn nghĩa là state càng bị ràng buộc chặt -> Ưu tiên duyệt.
        ...
        # Trả về giá trị âm hoặc nghịch đảo để A* ưu tiên f_cost nhỏ
        return -eliminated
    ...
    def h3_mrv_domain_size(self, state: FutoshikiState) -> int:
        ...
        for row, col in blanks:
            ...
            if domain_size == 0:
                return float("inf")  # Ngõ cụt vô nghiệm
        ...
        for row, col in blanks:
            ...
            if domain_size == 0:
                return (
                    -1,
                    -1,
                )  # PHÁT HIỆN NGÕ CỤT: Trả về cờ hiệu cắt nhánh ngay lập tức
    ...
    def get_successors(
        ...
        # Nếu nhánh này đã chết (ô trống không có số nào điền được)
        if row == -1:
            return successors

        row_used = set(state.grid[row, :])
        col_used = set(state.grid[:, col])
        used = (row_used | col_used) - {0}
        ...
        for value in range(1, self.size + 1):
            if value not in used:
                ...
                if not self.has_conflict(new_state):
