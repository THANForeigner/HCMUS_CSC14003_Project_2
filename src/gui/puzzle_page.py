...
class PuzzlePage(ft.View):
    def __init__(self, page: ft.Page):
        ...
        # UI Elements
        self.status_text = ft.Text("", size=18, weight=ft.FontWeight.BOLD, color=Win7Theme.TEXT_PRIMARY)
        self.board_container = ft.Container(padding=20, expand=True)
        self.numpad_container = ft.Container(
            content=ft.Text("Select a cell", color=Win7Theme.TEXT_SECONDARY, italic=True),
            width=200,
            padding=20,
        ...
        self.size_dropdown = ft.Dropdown(
            label="Size",
            width=100,
            label_style=ft.TextStyle(size=12, color=Win7Theme.TEXT_SECONDARY, weight=ft.FontWeight.BOLD),
            text_style=ft.TextStyle(size=13, color=Win7Theme.TEXT_PRIMARY, weight=ft.FontWeight.W_500),
        ...
        self.speed_slider = ft.Slider(min=0.01, max=1.0, value=0.1, width=120, on_change=self.on_speed_change, active_color=Win7Theme.PRIMARY)
        ...
        self.controls = [
            ft.Container(
                content=ft.Column([
                    ft.Row([
                        ft.IconButton(icon=ft.Icons.ARROW_BACK, on_click=lambda _: page.go("/"), icon_color=Win7Theme.TEXT_PRIMARY),
                        ft.VerticalDivider(width=1, color=Win7Theme.PANEL_BG),
                        ft.Container(
                            content=ft.Row([
                                ft.Text("Test Input:", size=14, color=Win7Theme.TEXT_SECONDARY, weight=ft.FontWeight.BOLD),
                                self.size_dropdown,
                                self.diff_dropdown,
                                self.id_dropdown,
                                ft.ElevatedButton(
                        ...
                        ft.VerticalDivider(width=1, color=Win7Theme.PANEL_BG),
                        ft.Text("Algo:", size=14, color=Win7Theme.TEXT_SECONDARY, weight=ft.FontWeight.BOLD),
                        ...
                        ft.TextButton("Clear", on_click=self.clear_board, style=ft.ButtonStyle(color=Win7Theme.ERROR))
        ...
        for r in range(self.size):
            ...
            for c in range(self.size):
                ...
                cell_content = ft.Text(
                    ...
                    color=Win7Theme.CELL_TEXT_FIXED if is_fixed else Win7Theme.CELL_TEXT_EMPTY
                )
                ...
                if c < self.size - 1:
                    ...
                    row_controls.append(ft.Container(content=ft.Text(txt, size=18, color=Win7Theme.CONSTRAINT, weight=ft.FontWeight.BOLD), alignment=ft.alignment.Alignment(0, 0), width=25))
            ...
            if r < self.size - 1:
                ...
                for c in range(self.size):
                    ...
                    v_controls.append(ft.Container(content=ft.Text(txt, size=18, color=Win7Theme.CONSTRAINT, weight=ft.FontWeight.BOLD), alignment=ft.alignment.Alignment(0, 0), width=55))
    ...
    async def on_cell_click(self, r, col_idx):
        ...
        for row in self.cells:
            for cell in row:
                if cell.border: cell.border = ft.border.all(1, Win7Theme.PANEL_BG)
        ...
        self.numpad_container.content = ft.Column([
            ft.Text(f"Cell ({r+1},{col_idx+1})", size=16, weight=ft.FontWeight.BOLD, color=Win7Theme.TEXT_SECONDARY),
    ...
    async def on_numpad_select(self, val):
        if self._active_cell:
            ...
            self.numpad_container.content = ft.Text("Select a cell", color=Win7Theme.TEXT_SECONDARY, italic=True)
        ...
        for r in range(self.size):
            for c in range(self.size):
                if not self.cells[r][c].content.value:
                    self.status_text.value = "Incomplete board"; self.status_text.color = Win7Theme.WARNING
                    self.page.update(); return
        self.status_text.value = "Perfect Solution!"; self.status_text.color = Win7Theme.SUCCESS
        self.page.update()

    async def solve_puzzle(self, e):
        async def on_result(solution, stats, steps=None):
            if solution is None:
                self.status_text.value = "No solution found"; self.status_text.color = Win7Theme.ERROR
            else:
                self.status_text.value = f"Solved: {stats.get('nodes_generated','?')} nodes"; self.status_text.color = Win7Theme.SUCCESS
                for r in range(self.size):
                    for c in range(self.size):
        ...
        async with asyncio.TaskGroup() as tg:
            ...
            tg.create_task(self.solver.run_with_history(self.size, self.grid_data, self.h_constraints, self.v_constraints, callback=on_result, algorithm=self.algorithm_dropdown.value, step_player=self.step_player))
    ...
    async def on_solve_instantly(self, e):
        ...
        async def on_result(solution, stats):
            if solution is None:
                ...
                self.status_text.value = f"Solved: {stats.get('nodes_generated','?')} nodes | {stats.get('time',0):.3f}s"; self.status_text.color = Win7Theme.SUCCESS
                for r in range(self.size):
                    for c in range(self.size):
                        if self._original_grid[r][c] == 0:
                            self.cells[r][c].content.value = str(solution[r][c])
        ...
        async with asyncio.TaskGroup() as tg:
            tg.create_task(self.solver.run_full(self.size, self.grid_data, self.h_constraints, self.v_constraints, callback=on_result, algorithm=self.algorithm_dropdown.value))
